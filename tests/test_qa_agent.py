...
def test_qa_agent_returns_valid_structure(monkeypatch):
    ...
    def fake_pull_requests(state="open", limit=20):
        return [
            {
                "number": 1,
                "title": "Old PR",
    ...
    assert result["review_priority_queue"][0]["number"] == 1  # older PR should rank first
...
def test_qa_agent_handles_api_failure(monkeypatch):
    def fake_failure(state="open", limit=20):
